...
rule plots:
    input:
        sorted_inferred_cnvs_heatmap = os.path.join(analysis_path, "inferred_cnvs", analysis_prefix) + "__cluster_tree_sorted_cnvs_bins_{root}.png",
        lib_sizes = os.path.join(analysis_path, "tree_learning", analysis_prefix) + "__clone_lib_sizes_{root}.png",
        cluster_tree_genes_figure =  os.path.join(analysis_path, "tree_learning", analysis_prefix) + "__cluster_tree_genes_{root}.png",
        overlapping_cluster_plot = os.path.join(analysis_path, "inferred_cnvs", analysis_prefix) + "__cluster_profile_overlapping_{root}.png",
...
rule plot_cnv_matrix:
    input:
        ...
        candidate_segmented_regions = os.path.join(analysis_path, "breakpoint_detection", analysis_prefix) + "_segmented_candidate_regions.txt",
        final_segmented_regions = os.path.join(analysis_path, "breakpoint_detection", analysis_prefix) + "_segmented_final_regions.txt",
        ...
        inferred_cnvs = os.path.join(analysis_path, "tree_learning", analysis_prefix) + "__cluster_tree_inferred_cnvs_final_{root}.csv",
        ...
        cell_labels = os.path.join(analysis_path, "tree_learning", analysis_prefix) + "__cell_labels_final_{root}.csv",
    output:
        sorted_normalised_counts_heatmap_candidate_bp = os.path.join(analysis_path, "inferred_cnvs", analysis_prefix) + "__cluster_tree_sorted_normalised_counts_bins_candidate_bps_{root}.png",
        sorted_normalised_counts_heatmap_final_bp = os.path.join(analysis_path, "inferred_cnvs", analysis_prefix) + "__cluster_tree_sorted_normalised_counts_bins_final_bps_{root}.png",
        sorted_normalised_counts_heatmap = os.path.join(analysis_path, "inferred_cnvs", analysis_prefix) + "__cluster_tree_sorted_normalised_counts_bins_{root}.png",
        sorted_inferred_cnvs_heatmap = os.path.join(analysis_path, "inferred_cnvs", analysis_prefix) + "__cluster_tree_sorted_cnvs_bins_{root}.png"
    run:
...
rule visualise_trees:
    ...
    input:
        cluster_tree = os.path.join(analysis_path, "tree_learning", analysis_prefix) + "__cluster_tree_final_{root}.txt",
        cell_node_assignments = os.path.join(analysis_path, "tree_learning", analysis_prefix) + "__cluster_tree_cell_node_ids_final_{root}.tsv",
        cluster_tree_json = os.path.join(analysis_path, "tree_learning", analysis_prefix) + "__cluster_tree_final_{root}.json",
        bin_gene_region_df_path = os.path.join(analysis_path, "inferred_cnvs", analysis_prefix) + "__bin_gene_region_df_{root}.csv",
        genes_to_highlight_path = disease_genes_path
    output:
        cluster_tree_graphviz = os.path.join(analysis_path, "tree_learning", analysis_prefix) + "__cluster_tree_{root}.graphviz",
        cluster_tree_figure =  os.path.join(analysis_path, "tree_learning", analysis_prefix) + "__cluster_tree_{root}.png",
        cluster_tree_genes_graphviz = os.path.join(analysis_path, "tree_learning", analysis_prefix) + "__cluster_tree_genes_{root}.graphviz",
        cluster_tree_genes_figure =  os.path.join(analysis_path, "tree_learning", analysis_prefix) + "__cluster_tree_genes_{root}.png"
    run:
        # Node labels
        ...
        # if wildcards.root == 'tetraploid':
        #     neutral_label = 'Tetraploid'

        # Tree with region labels
        regions_tree = tree_to_graphviz(input.cluster_tree, node_sizes=node_sizes, node_labels=node_labels, gender=params.gender, neutral_label=neutral_label)
        ...
        genes_tree = tree_to_graphviz(input.cluster_tree, node_sizes=node_sizes, gene_labels=True, bin_gene_region_df=bin_gene_region_df,
                                ...
                                gender=params.gender, neutral_label=neutral_label)
...
rule plot_cluster_cnvs:
    ...
    input:
        inferred_cnvs = os.path.join(analysis_path, "inferred_cnvs", analysis_prefix) + "__unique_cnvs_final_{root}.csv",
    ...
    run:
        ...
        if wildcards.root == 'tetraploid':
            ploidy = 4
        ymax = ploidy + params.max_amp_val + 1

        output_path = os.path.join(analysis_path, "inferred_cnvs", analysis_prefix)
        plot_cluster_cnvs(cnvs_arr, chr_stops, output_path=output_path, ymax=ymax, wildcard="_" + wildcards.root, offset_sizes=params.offset_sizes, s=params.s)
...
rule plot_heatmaps:
    input:
        ...
        bin_gene_region_df_path = os.path.join(analysis_path, "inferred_cnvs", analysis_prefix) + "__bin_gene_region_df_{root}.csv"
    output:
        gene_cn_df = os.path.join(analysis_path, "inferred_cnvs", analysis_prefix) + "__cn_gene_df_{root}.csv",
        roche_gene_cn_df = os.path.join(analysis_path, "inferred_cnvs", analysis_prefix) + "__cn_gene_df_{root}_roche_gene_list.csv",
        heatmap_cnvs = os.path.join(analysis_path, "inferred_cnvs", analysis_prefix) + "__heatmap_cnvs_{root}.png"
    run:
