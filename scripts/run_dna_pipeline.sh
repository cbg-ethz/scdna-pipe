...
out_path="/cluster/work/bewi/ngs/projects/tumorProfiler/analysis/trial_${2}/${1}-T/singlecell_dna/snake_analysis_files/" 
config_path="${out_path}config.json"
bsub -J ${1} -n 48 -W 23:57 -R fullnode -R "rusage[mem=5000]" snakemake -s /cluster/work/bewi/ngs/projects/tumorProfiler/code/dna-pipeline-novaseq/pipeline/Snakefile --configfile ${config_path} -j 48 -p -k -o ${out_path}
