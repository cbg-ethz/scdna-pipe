...
parser.add_argument(
    "-m", "--mat", required=True, help="The matrix containing original bins"
)
parser.add_argument("-r", "--reg_sizes", required=True, help="Region sizes file")
parser.add_argument(
    "-o", "--output_path", required=False, default="./", help="path to the output"
)
parser.add_argument(
    "-s", "--sample_name", required=False, default="", help="name of the sample"
)
...
np.savetxt(
    args.output_path + "/" + args.sample_name + "_segmented_counts.tsv",
    reg_avg_per_bin,
    delimiter="\t",
)
